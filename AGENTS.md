...
> Generated by `fpf init` on 2026-01-15 FPF Patterns: Proxy-Audit Loop, MVE
> Mandate, State Explicitness, Boundary Discipline Matrix
...
This agent system is **self-evolving**. See `.agents/roles.yaml` for
Meta-Strategist role.

### 8.1 Feedback Collection

After significant tasks, create a FeedbackRecord (schema in
`.agents/feedback-schema.yaml`):
