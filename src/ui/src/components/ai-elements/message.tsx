...
import { normalizeStreamdownMarkdown } from "@/lib/streamdown";
...
export const MessageResponse = memo(
  ({ className, children, ...props }: MessageResponseProps) => {
    const normalizedChildren = typeof children === "string"
      ? normalizeStreamdownMarkdown(children)
      : children;
        ...
        : (
          <>
            ...
            {onRemove && (
              <Button
                aria-label="Remove attachment"
                className="absolute top-2 right-2 size-6 shrink-0 rounded-full bg-background/80 p-0 opacity-0 backdrop-blur-sm transition-opacity hover:bg-background group-hover:opacity-100 [&>svg]:size-3"
                onClick={(e) => {
                  e.stopPropagation();
                  onRemove();
                }}
