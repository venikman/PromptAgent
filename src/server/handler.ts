...
export function createApiHandler(config: ApiConfig) {
        ...
        if (Array.isArray(parsed)) {
          storyPack = {
            ...
            userStories: parsed.map((item: Record<string, unknown>) => {
              ...
              const soThatMatch = desc.match(/\*\*so that\*\*\s*([^,*]+)/i) ||
  ...
  if (url.pathname === "/generate" && req.method === "POST") {
    ...
    const requestBody = {
      ...
      messages,
      stream: false,
      temperature: (payload?.options as Record<string, unknown>)?.temperature ??
        0.7,
      max_tokens: -1,
    };
  ...
  if (url.pathname === "/v2/playground" && req.method === "POST") {
    ...
    if (payload?.maxTokens !== undefined && maxTokens === undefined) {
      return jsonResponse(
        {
          error: `maxTokens must be an integer between ${MIN_MAX_TOKENS} and ${MAX_MAX_TOKENS}`,
        },
        400,
      );
    }
  ...
  if (url.pathname === "/v3/optimize" && req.method === "POST") {
    ...
    (async () => {
      ...
      try {
        ...
        const baselineObjective = task.progress.history[0]?.championObjective ??
          0;
        task.result = {
