...
Deno.test("Deno - should have @std imports", async () => {
  ...
  const hasStdImports =
    imports && Object.keys(imports).some((k) => k.startsWith("@std/"));
...
Deno.test("API - Vite proxy should cover API routes", async () => {
  const deployMainPath = join(ROOT_DIR, "deploy", "main.ts");
  const viteConfigPath = join(ROOT_DIR, "ui", "vite.config.ts");

  const deployMainContent = await readFileIfExists(deployMainPath);
  const viteConfigContent = await readFileIfExists(viteConfigPath);

  if (!deployMainContent || !viteConfigContent) return;

  // Extract route patterns from deploy/main.ts
  const routeMatches = deployMainContent.matchAll(
    /app\.(get|post|put|delete|patch)\s*\(\s*["'](\/?[^"']+)["']/gi,
  );

  const routes = [...routeMatches].map((m) => m[2]);

  // Extract top-level path prefixes
  ...
  // Check each prefix is in Vite proxy config
  for (const prefix of prefixes) {
    // Skip static files and root
    if (prefix === "/static" || prefix === "/") continue;

    const inProxy =
      viteConfigContent.includes(`"${prefix}"`) ||
      viteConfigContent.includes(`'${prefix}'`);

    if (!inProxy) {
      console.warn(
        `  ⚠ Route prefix "${prefix}" may not be proxied in Vite config`,
      );
    }
  }
});
