...
} from "../../src/orchestrator/optimization-progress.ts";
import type { IterationResult } from "../../src/orchestrator/types.ts";
...
Deno.test("OptimizationProgress: STEP_LABELS has all expected steps", () => {
  ...
  for (const step of expectedSteps) {
    assertEquals(
      typeof STEP_LABELS[step as keyof typeof STEP_LABELS],
      "string",
    );
  }
});
