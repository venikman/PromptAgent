...
import { cosine, hashVector, textSimilarity } from "../src/similarity.ts";
...
Deno.test("textSimilarity - similar texts have high similarity", () => {
  ...
  const textB =
    "The user needs to log in to the system with their email address";
...
Deno.test("textSimilarity - user story similarity detection", () => {
  ...
  assert(
    simAB > simAC,
    `Similar stories (A-B) should be more similar than different stories (A-C). AB=${simAB}, AC=${simAC}`,
  );
});
